...
procedure Calculation(var f: PUzel; var t: text);
...
procedure Calculation(var f: PUzel; var t: text); // вычисления 
var
  z1: PUzel; 
begin
 while f <> nil do
  begin
   ...
   while z1 <> nil do
    begin
     if (f^.x = z1^.x) and (f<>z1) then
      begin
       writeln(t, 'Да,  имеются совпадающие числа');
       exit;
      end;
     z1 := z1^.next;
    end;
 ...
 writeln(t, 'Нет, не имеются совпадающие числа');
end;
   end.
